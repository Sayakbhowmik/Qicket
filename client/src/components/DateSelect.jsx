            ...
            <div className='grid grid-cols-3 md:flex flex-wrap md:max-w-lg gap-4'>
              {Object.keys(dateTime).map((date) => (
                <button onClick={()=> setSelected(date)}
                  key={date}
                  className={`flex flex-col items-center justify-center h-14 w-14 rounded cursor-pointer bg-pink/5 hover:bg-pink/10 transition ${selected ===date ? "bg-primary text-white":
